...
class App extends Component {
  ...
  isImgMatched = id => {
    ...
    if (randomImgId === id) {
      this.setState(prevState => ({
        ...
        randomImgId: imagesList[randomIndex].id,
      }))
    } else {
      this.clearTimerInterval()
      this.setState({isSameImg: false})
  ...
  appHomePage = () => {
    const {tabId, randomImgId} = this.state

    const indexValue = imagesList.findIndex(eachObj => {
      if (eachObj.id === randomImgId) {
        return true
      }
      return false
    })

    const randomObj = imagesList[indexValue]

    const {imageUrl, category} = randomObj

    const filteredImgList = imagesList.filter(each => each.category === tabId)
        ...
        <ul className="images-list">
          {filteredImgList.map(eachImg => (
            <ImgTabs
              imgDetails={eachImg}
              key={eachImg.id}
              isImgMatched={this.isImgMatched}
            />
          ))}
        </ul>
      </div>
  ...
  greetingContainer = () => {
    ...
    return (
      <div className="greeting-container">
        ...
        <p className="greeting-card-title">YOUR SCORE</p>
        <h1 className="final-score">{score}</h1>
        <button className="reset-btn" type="button" onClick={this.resetGame}>
          <img
  ...
  render() {
    const {score, timeInSec, isSameImg} = this.state

    return (
      <div className="bg-container">
        <Navbar score={score} timeInSec={timeInSec} />
        {isSameImg ? this.appHomePage() : this.greetingContainer()}
      </div>
    )
  }
}
